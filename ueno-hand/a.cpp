...
double colorDiff(uint32_t x, uint32_t y) {
    double diff = 0;
    for (int i = 0; i < 3; i++) {
        auto d = abs(int(((uint8_t * ) & x)[i]) - int(((uint8_t * ) & y)[i]));
        diff += d * d;
    }
    return sqrt(diff);
}
...
void solve() {
	string blockId = "0";
	int globalCounter = 0;
	vector<int> ipos {400,380,363,322,307,297,278,268,254,234,215,186,171,146,134,115,96,84,73,58,40,24,0};
    ...
    map<int, uint32_t> lastColors, lastColorsNext;
    lastColors[400] = std::numeric_limits<uint32_t>::max();
	...
	for (int p = 1; p < (int)ipos.size(); ++ p) {
		...
		for (int j1 = 400; j1 > 0; ) {
            ...
            {
                // check right is same
                auto j0_it = lastColors.upper_bound(j0);
                auto j1_it = lastColors.upper_bound(j1-1);

                right_same = j0_it == j1_it;
                upperColor = j1_it->second;
            }

            bool left_same = false;
            {
                // check left is same
                auto j0_it = lastColors.lower_bound(j0+1);
                auto j1_it = lastColors.lower_bound(j1);

                left_same = j0_it == j1_it;
            }

            bool skip = false;
            if (left_same && right_same) {
                auto diff = colorDiff(upperColor, cc);
                if (diff * (j1-j0) * 0.005 < 5) {
                    skip = true;
                }
            }
        ...
        lastColors = std::move(lastColorsNext);
        lastColorsNext = map<int, uint32_t>();

        /*
        for (auto i = lastColors.begin(); i != lastColors.end(); ++i) {
            int merge = i->first;
            for (auto j = std::next(i); j != lastColors.end(); ++j) {
                auto d = colorDiff(i->second, j->second);
                if (d < 30) {
                    merge = j->first;
                } else {
                    break;
            ...
            lastColorsNext[merge] = i->second;
            i = lastColors.find(merge);
        }
        */

        /*
        lastColors = lastColorsNext;
        for (auto i : lastColors) {
            uint8_t* x = (uint8_t*)&i.second;
            std::cout << i.first << " color " << "["<<(int)x[0]<<","<<(int)x[1]<<","<<(int)x[2]<<","<<(int)x[3]<<"]"<<endl;
        }
        lastColorsNext = map<int, uint32_t>();
        */
	}
