    ...
    for (int k = 0; k < s.bs.size(); ++k) {
      ...
      if (memo.count(b.hash())) {
        ...
        for (int j = b.mnj + 1; j < b.mxj; ++j) {
          ...
          sim += calcCost(TYPE_LINE_CUT, b);
          if (sim < sim0) {
            best = {j, LINE_CUT_TYPE_J};
            sim0 = sim;
          }
        }
      }
      if (best.first == -1) continue;
      State t = lineCutBlock(s, k, best.first, best.second);
      memo[b.hash()] = best;
      v.push_back(t);
    }
    return v;
...
map<str, int> memoI;
map<str, int> memoJ;
vec<State> oneMove(const State& s)
{
  // if (s.validate()) assert(s.validate());
  vec<State> v;
  for (int k = 0; k < s.bs.size(); ++k) {
    ...
    if (memoI.count(H) && memoI[H] != -1) {
      u.push_back(lineCutBlock(s, k, memoI[H], LINE_CUT_TYPE_I));
    } else {
      memoI[H] = -1;
      double e = 1e50;
      for (int i = b.mni + 1; i < b.mxi; ++i) {
        u.push_back(lineCutBlock(s, k, i, LINE_CUT_TYPE_I));
        double d = u.back().ifColorAvLastN(2);
        if (d < e) {
          d = e;
          memoI[H] = i;
        }
    ...
    if (memoJ.count(H) && memoJ[H] != -1) {
      u.push_back(lineCutBlock(s, k, memoJ[H], LINE_CUT_TYPE_J));
    } else {
      memoJ[H] = -1;
      double e = 1e50;
      for (int j = b.mnj + 1; j < b.mxj; ++j) {
        u.push_back(lineCutBlock(s, k, j, LINE_CUT_TYPE_J));
        double d = u.back().ifColorAvLastN(2);
        if (d < e) {
          d = e;
          memoJ[H] = j;
    ...
    sort(u.begin(), u.end(), [] (auto a, auto b) { return a.ifColorAvLastN(2) < b.ifColorAvLastN(2); });
    while (3 < u.size()) u.pop_back();
    each (k, u) v.push_back(k);
  }
  return v;
}
...
void solve(void)
{
  ...
  for (int _ = 0; _ < 40 && q.size(); ++_) {
    ...
    while (q.size()) {
      ...
      // auto t = oneMove(q.top());
      auto t = johniel3::cutOneBlock(q.top());
      q.pop();
      each (k, t) {
        str h = k.hash();
