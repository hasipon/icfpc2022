...
struct State {
  ...
  double ifColorAv(void) const {
    double c = cost;
    double s = sim;
    each (b, bs) {
      double x = b.simulality();
      double y = calcCost(TYPE_COLOR, b);
      double w = b.simulalityAv();
      if (w > x + y) {
        c += y;
        s -= w;
        s += x;
...
State colorBlock(const State& s, int nth, Color c)
{
  State t = s;
...
State colorBlockAv(const State& s, int nth)
{
...
{
  State t = s;
  t.cost += calcCost(TYPE_LINE_CUT, t.bs[nth]);
  t.sim -= t.bs[nth].simulality();
  auto p = t.bs[nth].lineCut(k, type);
  t.cmd.push_back(p.first);
...
void solve(void)
{
  ...
  for (int _ = 0; _ < 50 && q.size(); ++_) {
    clog << _ << "th gen queue size: " << q.size() << ", " << "best.score=" << best.sim<<"+"<< best.cost << endl;
