...
int globalCounter = 0;
string paint2(string blockId, int i1, int j1, uint32_t c) {
	if (i1 == 399) {
		if (j1 == 399) {
			col(blockId, c);
			return blockId;
		} else {
			cout << "cut ["<<blockId<<"] [X] ["<<(j1+1)<<"]" << endl;
			col(blockId+".0", c);
		...
		if (j1 == 399) {
			cout << "cut ["<<blockId<<"] [Y] ["<<(i1+1)<<"]" << endl;
			col(blockId+".0", c);
			cout << "merge ["<<blockId<<".0] ["<<blockId<<".1]" << endl;
			return to_string(++globalCounter);
		} else {
			cout << "cut ["<<blockId<<"] ["<<(j1+1)<<","<<(i1+1)<<"]" << endl;
			col(blockId+".0", c);
			cout << "merge ["<<blockId<<".0] ["<<blockId<<".1]" << endl;
			auto r2 = to_string(++globalCounter);
			cout << "merge ["<<blockId<<".2] ["<<blockId<<".3]" << endl;
			auto r3 = to_string(++globalCounter);
			cout << "merge ["<<r2<<"] ["<<r3<<"]" << endl;
			return to_string(++globalCounter);
		}
	}
}
string paint(string blockId, int i0, int i1, int j0, int j1, uint32_t c) {
	if (i0 == 0) {
		if (j0 == 0) {
			return paint2(blockId, i1, j1, c);
		} else {
			cout << "cut ["<<blockId<<"] [X] ["<<j0<<"]" << endl;
			auto r = paint2(blockId+".1", i1, j1, c);
			cout << "merge ["<<blockId<<".0] ["<<r<<"]" << endl;
			return to_string(++globalCounter);
		}
	} else {
		if (j0 == 0) {
			...
			cout << "cut ["<<blockId<<"] ["<<j0<<","<<i0<<"]" << endl;
			auto r = paint2(blockId+".2", i1, j1, c);
			cout << "merge ["<<blockId<<".0] ["<<blockId<<".1]" << endl;
			auto r2 = to_string(++globalCounter);
			cout << "merge ["<<r<<"] ["<<blockId<<".3]" << endl;
			auto r3 = to_string(++globalCounter);
			cout << "merge ["<<r2<<"] ["<<r3<<"]" << endl;
			return to_string(++globalCounter);
		}
...
void solve() {
	...
	for (int i = 60; i < 400; i += 40) {
		for (int j = 20; j < 360; j += 40) {
			...
			blockId = paint(blockId, I0, I1, J0, J1, cc);
		}
	}
	double score = 0;
	for (int i = 0; i < 400; ++ i) {
